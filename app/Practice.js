...
export default function Practice({ onDone }) {
  ...
  const handleResponse = (userResponse) => {
    ...
    const result = isCorrect
      ...
      : "Incorrect. The red triangle was there, so the correct response was: Y";

    setResponses((prev) => [
      ...prev,
      { image: array[currentIndex].src, userResponse, isCorrect },
    ]);
    ...
    setTimeout(() => {
      ...
      if (currentIndex < array.length - 1) {
        ...
        // When practice ends, move to next component
        if (onDone) onDone();
      }
    }, 1000);
  };
